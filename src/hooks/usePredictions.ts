...
import { BRACKET_DEADLINE, players, TOTAL_GAMES } from "@/data/players";
...
export function getPickablePlayersForGame(
	...
	if (gameId.startsWith("r1-")) {
		...
		// Round 1 matchups: players[0] vs players[1], players[2] vs players[3], etc.
		const player1Index = gameIndex * 2;
		const player2Index = gameIndex * 2 + 1;
		return [players[player1Index]?.id, players[player2Index]?.id];
	}
